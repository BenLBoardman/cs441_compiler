...
### Milestone 4
- The compiler now has full garbage collection support and some "enhanced error handling" - errors will now not be emitted until the end of the current compilation phase (parsing, type checking, CFG construction, optimizations) for most type-related errors and some errors related to syntax in method definitions. This is a proof-of-concept, and all other errors will be emitted immediately upon detection (and abort compilation). `gccrash.comp` has been created to show a program that runs out of memory in fixed-memory execution but crashes with a garbage collector. All relevant code changes for GC exist in `BasicBlock.java`, specifically lines 690-709 where parsed class references are processed and inserted into CFG. The `CFGClass.java` also contains a new function `getBitMap()`, which constructs the garbage collection bitmap for that class' fields. Other minor code changes, bug fixes, and refactors exist in various places (most notably, method calls now correctly pass the object address rather than the address of the VTable, an error introduced in erroneous code from Assignment 3).
- I also considered, but did not implement, the following modifications:
  - modular division support in the compiler and simulator
  - changing type annotations from `var:type` to `var$type` to simplify portions of error detection - when an untyped local variable in a method header is immediately followed with a variable reference in the first line of the method, it can be difficult for the compiler to detect the right kind of error
